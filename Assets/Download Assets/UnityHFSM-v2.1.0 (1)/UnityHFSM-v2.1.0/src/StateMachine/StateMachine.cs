...
namespace UnityHFSM
{
		...
		IActionable<TEvent>
	{
		...
		private StateBase<TStateId> previousState = null;
		...
		public StateBase<TStateId> PreviousState
		{
			get
			{
				EnsureIsInitializedFor("Trying to get the предыдущую state");
				return previousState;
			}
		}
		public TStateId PreviousStateName => PreviousState.name;
		...
		private void ChangeState(TStateId name, ITransitionListener listener = null)
		{
			...
			//тут состояние сохраняем как прошлое
